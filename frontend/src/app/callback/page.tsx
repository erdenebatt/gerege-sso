      ...
      <div className="glass rounded-2xl p-10 w-full max-w-[420px] text-center">
        ...
        {view === 'success' && user && (
          <div className="py-5">
            ...
            <h2 className="text-xl font-semibold mb-5">Амжилттай нэвтэрлээ!</h2>

            <div className="bg-white/5 rounded-xl p-4 text-left mb-5">
              <div className="flex justify-between py-2 border-b border-white/10">
                <span className="text-white/60 text-sm">Gen ID:</span>
                <span className="font-medium text-sm">{user.gen_id}</span>
              </div>
              <div className="flex justify-between py-2 border-b border-white/10">
                <span className="text-white/60 text-sm">И-мэйл:</span>
                <span className="font-medium text-sm">{user.email}</span>
              </div>
              <div className="flex justify-between py-2">
                <span className="text-white/60 text-sm">Баталгаажсан:</span>
                <span
                  className={`font-medium text-sm ${user.verified ? 'text-green-400' : 'text-orange-400'}`}
                >
              ...
              {user.gerege?.name && (
                <div className="flex justify-between py-2 border-t border-white/10">
                  <span className="text-white/60 text-sm">Нэр:</span>
                  <span className="font-medium text-sm">{user.gerege.name}</span>
                </div>
              )}
            </div>

            <div className="bg-black/20 rounded-lg p-3 text-left mb-5">
              <label className="text-xs text-white/60 mb-2 block">JWT Token:</label>
              <div className="flex items-center gap-2">
                <code className="flex-1 font-mono text-xs text-gerege-primary bg-gerege-primary/10 p-2 rounded break-all">
                  {truncatedToken}
                </code>
                <button
                  onClick={copyToken}
        ...
        {view === 'verify' && (
          <div className="py-5">
            ...
            <div className="bg-orange-500/10 border border-orange-500/30 rounded-xl p-5 text-left mb-5">
              <h3 className="font-medium mb-2">Иргэний баталгаажуулалт</h3>
              ...
              {verifySuccess && (
                <div className="mt-3 p-3 bg-green-500/20 border border-green-500/50 rounded-lg text-green-400 text-sm">
                  {verifySuccess}
                </div>
              )}
            </div>
        ...
        {view === 'pending_link' && (
          <div className="py-5">
            ...
            <h2 className="text-xl font-semibold mb-3">Бүртгэл холбох</h2>
            ...
            <p className="text-xs text-white/50">
              ...
              <Link href="/" className="text-gerege-secondary hover:underline">
                буцах
              </Link>
            </p>
          </div>
        ...
        {view === 'error' && (
          <div className="py-5">
            <div className="w-16 h-16 rounded-full bg-red-500/20 text-red-400 text-3xl flex items-center justify-center mx-auto mb-5">
              ✗
            </div>
            <h2 className="text-xl font-semibold mb-3">Алдаа гарлаа</h2>
            <p className="text-white/70 mb-5">{errorMessage}</p>
            <Link
              href="/"
              className="inline-block px-6 py-3 bg-white text-gerege-dark rounded-xl font-medium hover:-translate-y-0.5 hover:shadow-lg transition-all"
            >
              Дахин оролдох
            </Link>
          </div>
